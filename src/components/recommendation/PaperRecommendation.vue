<template>
  <el-card shadow="always">
      <div class="rec-paper-header">
          ...
          <el-link type="primary" @click="viewMore" class="paper-title">{{showSize===3?'View More':'Pack Up'}}</el-link>
      </div>
      <div v-for="(item, index) in papers.slice(0,showSize)" :key="index" class="paper-item">
          ...
          <mul-wrapper :index=item.id :content=item.paperAbstract></mul-wrapper>
